...
$reponse = $bdd->query('SELECT * FROM ventes_encheres WHERE (choice="vente" AND date_bd >= CURDATE() ) ORDER BY date_bd ASC');
        ...
        <p class="mb-5" id="fonts_write">
          ...
          if ($validityOfReq != 0) {
...

            {
                ...
                <div class="col-md-12">
                  <div class="row">
                    <div class="col-12">
                      ...
                      <p class="title-desc"><img src="img/ic_time.svg"/> <?php echo /*utf8_encode(strftime('%d %B %Y', strtotime($post->$donnees['date_bd'])));*/strftime("%a %e %B %Y", strtotime($donnees['date_bd'])); ?> à <?php echo strftime("%H:%M", strtotime($donnees['heure'])); ?> <img src="img/ic_location.svg"/><?php echo $donnees['lieu']; ?>
                        <br><a href="https://www.google.com/maps/search/<?php echo $donnees['lieu']; ?>/" target="_blank">Cliquez ici pour accéder à l'adresse</a>
                      </p>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-12">
                      <div class="col-12 strg" style="padding:0;">
                        ...
                        if ($donnees['link_materiel'] != 'none'){
                          ?>
                          <p>Liste du matériel à vendre : <a href="<?php echo $donnees['link_materiel']; ?>" target="_blank">Cliquez ici</a><br /></p>
                          <?php
                        }
                        else{
                        }
                        ...
                        if ($donnees['link_cartesGrises'] != 'none'){
                          ?>
                          <p>Carte(s) grise(s) véhicule(s) : <a href="<?php echo $donnees['link_cartesGrises']; ?>" target="_blank">Cliquez ici</a><br /></p>
                          <?php
                        }
                        else{
                        }
                        ...
                        if ($donnees['link_controle_technique'] != 'none'){
                          ?>
                          <p>Contrôle technique : <a href="<?php echo $donnees['link_controle_technique']; ?>" target="_blank">Cliquez ici</a><br /></p>
                          <?php
                        }
                        else{
                        }
                      ...
                      $number = $donnees['num_page'];
                      $query = $bdd->query('SELECT * FROM images WHERE num_page = ' . $number);
                      ?>

                      <div class="row col-md-12">
                        <?php
                        while ($imgURL = $query->fetch())
                        {?>
                          <div class="" style="margin: 1em;">
                            <img class="d-block w-100" src="<?= "uploads/".$imgURL['file_name']?>" alt="" style="max-height: 300px; max-width: 300px;">
                          </div>
                        <?php }
                        ?>
                      </div>
              ...
              ?>
              <p class="title-ventes">Aucune vente à venir</p>
              <?php
            }?>
          </p>
